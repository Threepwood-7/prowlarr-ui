...
def test_recheck_and_deferred_recheck_are_noops_during_shutdown(window, monkeypatch):
    ...
    monkeypatch.setattr(window, "_recheck_everything_for_titles", lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_close_event_ignores_and_retries_when_workers_refuse_to_stop(window, monkeypatch):
    ...
    scheduled = {"count": 0}
    monkeypatch.setattr(window, "_schedule_timer", lambda _delay, _cb: scheduled.__setitem__("count", scheduled["count"] + 1))
...
def test_close_event_prompts_then_forces_exit_after_shutdown_deadline(window, monkeypatch):
    ...
    now = {"value": 0.0}
    monkeypatch.setattr("prowlarr_ui.time.monotonic", lambda: now["value"])
    scheduled = {"count": 0}
    monkeypatch.setattr(window, "_schedule_timer", lambda *_args, **_kwargs: scheduled.__setitem__("count", scheduled["count"] + 1))

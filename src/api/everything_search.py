...
def find_everything_exe() -> Optional[str]:
    ...
    try:
        import shutil
        path = shutil.which("Everything.exe")
        if path:
            logger.info(f"Found Everything.exe in PATH: {path}")
            return path
...
SDK_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), ".everything_sdk")
DEFAULT_SDK_URL = "https://www.voidtools.com/Everything-SDK.zip"
DLL_NAME = "Everything64.dll"


def _download_everything_sdk(sdk_url: str = DEFAULT_SDK_URL) -> Optional[str]:
...
def find_everything_dll(sdk_url: str = DEFAULT_SDK_URL) -> Optional[str]:
    """

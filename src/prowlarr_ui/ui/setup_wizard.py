...
class ProwlarrSetupWizardDialog(QDialog):
    def __init__(self, initial: dict[str, Any], parent: QWidget | None = None) -> None:
        ...
        self.txt_host = QLineEdit(
            str(prowlarr.get("host", defaults["prowlarr"]["host"]) or "")
        )
        ...
        self.txt_http_user = QLineEdit(
            str(
                prowlarr.get(
                    ...
                    defaults["prowlarr"]["http_basic_auth_username"],
                )
                or ""
            )
        )
        self.txt_http_password = QLineEdit(
            str(
                prowlarr.get(
                    ...
                    defaults["prowlarr"]["http_basic_auth_password"],
                )
                or ""
            )
        )
    ...
    def _on_accept(self) -> None:
        ...
        if not (host.startswith("http://") or host.startswith("https://")):
            QMessageBox.warning(
                self, "Validation", "Prowlarr host must start with http:// or https://"
            )
            return
    ...
    def _on_test_connection(self) -> None:
        ...
        if not (host.startswith("http://") or host.startswith("https://")):
            QMessageBox.warning(
                self,
                "Connection Test",
                "Prowlarr host must start with http:// or https://",
            )
            return
        if not api_key or api_key == "YOUR_API_KEY_HERE":
            QMessageBox.warning(
                self, "Connection Test", "Prowlarr API key is required."
            )
            return
